...
During routine traffic stops, officers have latitude to search both driver and vehicle for drugs, weapons, and other contraband when they suspect more serious criminal activity.
As a result, there have been concerns that racial bias plays a role in officers' decision.
Claims of biased decision making, however, are typically hard to rigorously assess.
This is in large part because of well-known problems with the two most common statistical tests for discrimination.
...
searches. 
Becker argued that even if searches of minorities yield contraband less often than searches of whites, it suggests that the bar for searching minorities
is lower, indicative of discrimination.
...
The two most common statistical tests for discrimination, termed "benchmarking" and "outcome" suffer from well-known limitations.
...
We overcome these limitations, we develop the following stylized model of officer behaviour during traffic stops.
...
For any stop $i$, the officer observes a random signal: 

$x_i \sim Beta(\phi_{rd} , \delta_{rd})$
...
In the two figures below, consider two hypothetical signal (solid curves) and search thresholds (dashed vertical
lines) that illustrate how the benchmark and outcome tests can give misleading results. Under the
model described above, the search rate for a given group is equal to the area under the signal distribution
...
the left plot, because red drivers face a lower search threshold than blue drivers. However, blue drivers are
subject to the lower threshold (right plot), contradicting the results of the benchmark and outcome tests.
...
$\delta_{rd} = P ( x_{rd}  > t_{rd}  ;  \phi_{rd}, \lambda_{rd} )$
...
$\delta_i =$ exp$(\delta_r + \delta_d)$


$\phi_i =$ logit$^{-1}(\phi_r + \phi_d)$

and place weak priors on the race-department parameters.
    
We next use our threshold test to infer race- and department-specific search thresholds using the model outlined below. 
Given the observed data, we estimate the posterior distribution of the search thresholds via Hamiltonian Monte Carlo (HMC) sampling.
We sample five Markov chains of 1,000 iterations (with the first half for warmup) in parallel, and found this was sucient for convergence, as indicated by $\hat{R}$ values less than 1.05 for all parameters.
...
The figure below displays the department-level thresholds for whites, blacks, Hispanics. 
...
Kenneth Arrow. The theory of discrimination. In Discrimination in Labor Markets. Princeton
