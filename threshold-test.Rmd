...
This in large part because of well-known problems with the two most common statistical tests for discrimination: benchmarking and outcome tests. 
...
For each stop $i$ with driver race $r$, and department $d$, we observe $N_{rd}$ stops

$S_{rd} = \sum_{i=1}^N S_i$  

$H_{rd} = \sum_{i=1}^N H_i$

The officer observes a random signal drawn from a race and department specific discriminant distribution (Pierson et. al, 2017),  and deterministically conducts search if their estimated probability exceeds the race and department-specific search threshold $t_{rd}$. 
...
Pierson, Emma, Sam Corbett-Davies, and Sharad Goel. "Fast threshold tests for detecting discrimination.", 2017.

Simoiu, Camelia, Sam Corbett-Davies, and Sharad Goel. The problem of infra-marginality in outcome tests for discrimination, 2017.
